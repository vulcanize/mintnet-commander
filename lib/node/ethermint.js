...
class EtheremintNode extends Node {
  ...
  static nodesReady (nodes) {
    ...
    let nodeCommands = nodes.map((node) => {
      ...
      runCommands.push(new commands.WriteFile('genesis.json', utils.payloadToString(tendermintGen)))
      runCommands.push(new commands.WriteFile('_.json', utils.payloadToString(ethermintGen)))
      runCommands.push(new commands.WriteFile('_.json', utils.payloadToString(ethermintGen)))
