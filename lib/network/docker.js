...
class DockerNetwork extends Network {
  constructor (NodeType, options = {}) {
    ...
    if (!options.serverRoot) {
      throw new Error('serverRoot option must be provided')
    }
  ...
  async setupMachines () {
    ...
    await DockerMachine.machinesReady(this.NodeType, this.machines, this.name)
  }
  ...
  async init (networkData, machines) {
    this.network = await this.docker.getNetwork(networkData.id)

    this.createMachines()
    this.machines.forEach((machine, idx) => {
      machine.init(machines[idx])
    })
  }
